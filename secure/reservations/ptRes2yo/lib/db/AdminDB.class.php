...
class AdminDB extends DBEngine {
	...
	*/
	function add_resource($rs, $curid=null, $id = null) {
		...
		$q = $this->db->prepare('SELECT * FROM ' . $this->get_table('resources') . ' WHERE address1=? and zip=? and state=? and city=?');
		$result = $this->db->execute($q, array($rs['address1'], $rs['zip'], $rs['state'], $rs['city']));
