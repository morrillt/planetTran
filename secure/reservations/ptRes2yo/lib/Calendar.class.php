...
class Calendar {
	...
	*/
	function printJumpForm() {
        ...
        ?>
        		<td style="width: 33%;" class="navMonthStyle"><a href="javascript: changeDate(<?= $prevMonth . ',' . $prevYear?>);"><?= "&lt; " . strftime("%b",mktime(0,0,0,$prevMonth,1,$prevYear))  ?></a></td>
        		<td style="width: 33%">
        ...
        ?>
				  ...
				  <input type="hidden" name="scheduleid" value="<?= $this->scheduleid?>" />
        		</td>
        		<td style="width: 33%;" class="navMonthStyle"><a href="javascript: changeDate(<?= $nextMonth . ',' . $nextYear?>);"><?= strftime("%b",mktime(0,0,0,$nextMonth,1,$nextYear)) . " &gt;" ?></a></td>
        	  </tr>
            </table>
          </form>
	<?
